const galleryCategories = [
  "Extensions",
  "Rénovation",
  "Commerces / Activités",
  "Bâtiments Professionnels"
];

const galleryProjects = [

  /* ================================================== */
  /* EXTENSIONS */
  /* ================================================== */

  /* #region Projet Extension bois */
  {
    category: "Extensions",
    title: "Extension bois",
    meta: "Extension • 2024",
    cover: "assets/images/galerie/extension-01.webp",

    images: [
      "assets/images/galerie/extension-01.webp",
      "assets/images/galerie/extension-02.webp"
    ],

    alt: "Extension en ossature bois",

    shortText:
      "Agrandissement d’une habitation existante afin de créer un nouvel espace de vie lumineux...",

    fullText:
      "Agrandissement d’une habitation existante afin de créer un nouvel espace de vie lumineux et contemporain. Le projet s’attache à dialoguer avec le bâti existant tout en affirmant une écriture actuelle et mesurée.",

    youtube: "https://www.youtube.com/embed/YdyQpB3Xp7w",

    videoTitle:
      "Présentation vidéo du projet"
  },
  /* #endregion Projet Extension bois */




  /* ================================================== */
  /* RÉNOVATION */
  /* ================================================== */

  /* #region Projet LR */
  {
    category: "Rénovation",
    title: "Projet LR",
    meta: "Rennes (35) • 2024 / 2025",
    cover: "assets/images/galerie/renovation/projet-lr/apres-4.webp",

    images: [
      {
        src: "assets/images/galerie/renovation/projet-lr/avant-1.webp",
        label: "Avant"
      },
      {
        src: "assets/images/galerie/renovation/projet-lr/pendant-1.webp",
        label: "Pendant"
      },
      {
        src: "assets/images/galerie/renovation/projet-lr/apres-1.webp",
        label: "Après"
      },

      {
        src: "assets/images/galerie/renovation/projet-lr/avant-2.webp",
        label: "Avant"
      },
      {
        src: "assets/images/galerie/renovation/projet-lr/apres-2.webp",
        label: "Après"
      },
      {
        src: "assets/images/galerie/renovation/projet-lr/apres-2-bis.webp",
        label: "Après"
      },

      {
        src: "assets/images/galerie/renovation/projet-lr/avant-3.webp",
        label: "Avant"
      },
      {
        src: "assets/images/galerie/renovation/projet-lr/apres-3.webp",
        label: "Après"
      },
      {
        src: "assets/images/galerie/renovation/projet-lr/apres-3-bis.webp",
        label: "Après"
      },

      {
        src: "assets/images/galerie/renovation/projet-lr/avant-4.webp",
        label: "Avant"
      },
      {
        src: "assets/images/galerie/renovation/projet-lr/apres-4.webp",
        label: "Après"
      }
    ],

    alt: "Projet LR rénovation maison de ville Rennes",

    shortText:
      "Rénovation complète d’une petite maison de ville située en cour intérieure à Rennes, repensée pour offrir de véritables espaces de vie familiaux.",

    fullText: `
        Maîtrise d’ouvrage : Privé
        Localisation : Rennes (35)
        Année de réalisation : 2024 / 2025
        Surface : 25 m²
        Durée étude : 16 mois
        Durée travaux : 11 mois
        (Chantier livré octobre 2025)


        Descriptif :

        Mission complète

        Ce projet se situe dans une rue piétonne à Rennes, une petite maison de ville dans la cour d’un immeuble.

        L’état des lieux : au RDC un garage inutilisable, au R+1 une pièce à vivre avec petite cuisine, au R+2 une chambre avec SDE.

        La cliente souhaitait une maison fonctionnelle pour elle et ses filles, avec de vrais espaces de couchage et de vie.

        Le choix s’est donc tourné vers une dépose totale des plateaux, la famille a dû se déménager le temps des travaux.

        La présence d’amiante a augmenté le temps de démolition/dépose.

        L’esquisse proposait de donner au RDC une réelle fonction de pièce à vivre avec un vrai espace de cuisine ; au R+1 une chambre parentale avec salle d’eau ouverte, dressing et toilettes indépendants ; au R+2 une chambre avec salle de bain indépendante équipée de sanitaires.
        `,
  },
  /* #endregion Projet LR */


  /* ================================================== */
  /* COMMERCES / ACTIVITÉS */
  /* ================================================== */
  /* #region Projet Aménagement intérieur */
  {
    category: "Commerces / Activités",
    title: "Aménagement intérieur",
    meta: "Commerce • 2024",
    cover: "assets/images/galerie/interieur-01.webp",

    images: [
      "assets/images/galerie/interieur-01.webp"
    ],

    alt: "Aménagement intérieur épuré",

    shortText:
      "Travail sur les ambiances, la circulation et la cohérence des matériaux...",

    fullText:
      "Travail sur les ambiances, la circulation et la cohérence des matériaux afin de créer des espaces intérieurs lisibles, apaisés et adaptés aux usages professionnels."
  },
  /* #endregion Projet Aménagement intérieur */



  /* ================================================== */
  /* BÂTIMENTS PROFESSIONNELS */
  /* ================================================== */

  /* #region Projet Étude conceptuelle */
  {
    category: "Bâtiments Professionnels",
    title: "Étude conceptuelle",
    meta: "Projet professionnel",
    cover: "assets/images/galerie/concours-01.webp",

    images: [
      "assets/images/galerie/concours-01.webp"
    ],

    alt: "Étude conceptuelle architecturale",

    shortText:
      "Proposition architecturale développée autour d’un concept fort et d’une écriture maîtrisée...",

    fullText:
      "Proposition architecturale développée autour d’un concept fort et d’une écriture maîtrisée. Le travail présente une démarche de conception complète, de l’idée à la traduction spatiale."
  }
  /* #endregion Projet Étude conceptuelle */


];